...
export const metadata = {
  title: 'Coin Type Visualization | Early Chinese Coin Finds',
  description:
    'Interactive map of georeferenced coin find sites with typology-based coin type filtering.',
}

export default async function CoinTypeVisualizationPage({ searchParams }: PageProps) {
      ...
      <div className="flex shrink-0 flex-wrap items-center gap-x-3 gap-y-1 border-b border-brand/20 bg-white px-3 py-1.5 sm:px-4">
        ...
        <div className="flex flex-wrap items-center gap-1.5 sm:ml-auto">
          ...
          {PRECISION_TABS.map((tab) => {
            ...
            const href = `/visualizations/coin-type${tab.id === 'all' ? '' : `?precision=${tab.id}`}`
            return (
              <Link
                key={tab.id}
                href={href}
