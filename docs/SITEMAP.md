...
- **Libs/data:** `lib/queries.ts` (`getMints`, `getFindsForHeatmap`, `getCoinIssues`), `lib/mint-directory.ts` (`buildMintDirectory` — flattens the live `mints` table, no static fallback layer), `lib/mint-stats.ts` (`computeMintStatsFromFinds`, `toMintPoints`), `lib/admin/guard.ts` (`isAuthorized`), `lib/admin/mints-actions.ts` (`createMint`)
...
- **Libs/data:** `lib/queries.ts` (`getMints`, `getMintFindspotsData`, `getAllSourceLinks`), `lib/mint-directory.ts` (`buildMintDirectory`, `getMintDirectoryEntryBySlug` — all fields, including images, references, and location notes, come straight from the `mints` row), `lib/admin/guard.ts` (`isAuthorized`), `lib/admin/mints-actions.ts` (`updateMint`)
